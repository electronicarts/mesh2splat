...
int determineFaceIndex(vec4 gaussianWs) {
    vec3 dir = normalize(gaussianWs.xyz - u_lightPos);
    vec3 absDir = abs(dir);
...
void main() {
	...
	int face = determineFaceIndex(gaussianWs);
    if(face < 0) return;

	vec4 gaussian_vs = u_worldToViews[face] * vec4(gaussianWs.xyz, 1);
	...
	else if (u_format == 1)
	{
		...
		vec3 rgbN = encodeNormal(rotMatrix[minCompIndex].xyz);
    ...
    mat3 W = mat3(u_worldToViews[face]);
    mat3 JW = J * W;
